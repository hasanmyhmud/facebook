...
public:
    ...
    void PrintTimeline(bool print)
    {
        ...
        for (int i = 0; i < numPosts; i++)
        {
            ...
            {
                cout << Title << ":" << timeline[i]->getpostid();
                cout << "                 \n";
    ...
    for (int i = 0; i < numPosts; i++)
    {
        ...
        if (postDate->getdate() == currentdate.getdate())
        {
            if (postDate->getmonth() == currentdate.getmonth())
            {
                ...
                {
                    cout << endl
                        << currentdate.getyear() - postDate->getyear() << " years ago" << endl;
                }
            }
            else
            {
                difference = currentdate.getmonth() - postDate->getmonth();
                if (difference < 0)
                    difference = 12 + difference;
                cout << endl
                    << difference << " months ago" << endl;
            }
        }
    }
}